...
static enum wlr_color_encoding color_encoding_to_wlr(
...
static enum wlr_color_range color_range_to_wlr(
...
static enum wlr_color_chroma_location chroma_location_to_wlr(
...
static void color_repr_handle_set_alpha_mode(struct wl_client *client,
	...
	color_repr->pending.alpha_mode = alpha_mode_to_wlr(alpha_mode);
}
...
static void color_repr_handle_set_coefficients_and_range(struct wl_client *client,
	...
	color_repr->pending.coefficients = color_encoding_to_wlr(coefficients);
	color_repr->pending.range = color_range_to_wlr(range);
}
...
static void color_repr_handle_set_chroma_location(struct wl_client *client,
	...
	if (!wp_color_representation_surface_v1_chroma_location_is_valid(
			WP_COLOR_REPRESENTATION_VERSION, chroma_location)) {
		// The protocol doesn't have an error to report a client value
		// which isn't even a valid enum, so just ignore it.
	...
	color_repr->pending.chroma_location = chroma_location_to_wlr(chroma_location);
}
...
static void send_supported(struct wlr_color_representation_manager_v1 *manager,
	...
	for (size_t i = 0; i < manager->supported_alpha_modes_len; i++) {
		enum wp_color_representation_surface_v1_alpha_mode *supported =
			&manager->supported_alpha_modes[i];
